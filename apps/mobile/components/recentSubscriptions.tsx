...
export default function RecentSubscriptions({ data }: { data: DashboardResponse }) {
    const { colors } = useAppTheme();

    if (!data.recentSubscriptions.length) return null;

    return (
        <View style={[styles.card, { backgroundColor: colors.card }]}>
            <Text style={[styles.title, { color: colors.text }]}>Recent Subscriptions</Text>
